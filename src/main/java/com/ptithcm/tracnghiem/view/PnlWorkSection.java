...
import com.ptithcm.tracnghiem.controller.BoDeController;
import com.ptithcm.tracnghiem.controller.GiaoVienDangKyController;
        ...
        tblData.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                try {
                    ...
                    switch (tabName) {
                        case "Quản lý bộ đề":
                            object = (BoDe) getObjectFromRow(selectedRow);
                            if (BoDeService.checkQuestionExistInAllTable(((BoDe) object).getCauHoi())) {
                                pnlManageBar.disableManageButtons("Not delete");
                            }
                            break;
                        case "Đăng ký thi":
                            ((PnlTeacherRegisterInfo) pnlObjectInfo).disableComponents();
                            object = (GiaoVienDangKy) getObjectFromRow(selectedRow);
                            GiaoVienDangKy gvdk = (GiaoVienDangKy) object;
                            if (GiaoVienDangKyService.checkGVDKExistInAllTable(gvdk.getMaLop(), gvdk.getMaMH(), gvdk.getLan())) {
                                pnlManageBar.disableManageButtons("Not update");
                                pnlManageBar.disableManageButtons("Not delete");
                            }
                            break;
                        default:
...
//                            Xử lý dữ liệu và thực hiện thao tác thêm
                            GiaoVienDangKy gvdk
                                    = pnlTeacherRegisterInfo.getGVDKData(objectActions);
                            int objectActionSize = objectActions.size();
                            DefaultTableModel tblDataModel = (DefaultTableModel) tblData.getModel();

                            ObjectAction objectAction = new ObjectAction(
                                    ActionStatusEnum.INSERT, objectActionSize,
                                    tblDataModel.getRowCount(), gvdk
                            );
...
//                            Cập nhật lại bảng
                            Object[] row = {
                                gvdk.getMaGV(), gvdk.getMaLop(), gvdk.getMaMH(),
                                gvdk.getTrinhDo(), new SimpleDateFormat("dd/MM/yyyy HH:mm").format(gvdk.getNgayThi()),
...
//                            Xử lý dữ liệu và thực hiện thao tác cập nhật
                            ...
                            int objectActionIndex = -1;
                            for (int i = 0; i < objectActions.size(); ++i) {
                                if (tblData.getValueAt(selectedRowIndex, 1).toString().equals(((GiaoVienDangKy) objectActions.get(i).getObjects().get(0)).getMaLop())
                                        && tblData.getValueAt(selectedRowIndex, 2).toString().equals(((GiaoVienDangKy) objectActions.get(i).getObjects().get(0)).getMaMH())
                                        && Short.parseShort(tblData.getValueAt(selectedRowIndex, 5).toString()) == ((GiaoVienDangKy) objectActions.get(i).getObjects().get(0)).getLan()) {
                                    objectActionIndex = i;
                                    break;
                                }
                            }
                            ...
                            if (objectActionIndex != -1) {
                                ObjectAction objectAction = new ObjectAction(
                                        ActionStatusEnum.UPDATE,
                                        objectActionIndex, selectedRowIndex,
                                        (GiaoVienDangKy) objectActions.get(objectActionIndex).getObjects().get(0)
                                );
...
//                        Ẩn/hiện các nút chức năng
                    ...
                    case "Đăng ký thi": {
                        ...
                        if (objectActionIndex != -1) {
                            ObjectAction objectAction = new ObjectAction(
                                    ActionStatusEnum.DELETE,
                                    objectActionIndex, selectedRowIndex, gvdk
                            );
...
//        Xử lý sự kiện nút save      
        pnlManageBar.getBtnSave().addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                switch (tabName) {
                    case "Quản lý bộ đề":
                        BoDeController.save(frmMain, _this, objectActions);
                        break;
                    case "Đăng ký thi":
                        GiaoVienDangKyController.save(frmMain, _this, objectActions);
                        break;
                    default:
                        throw new AssertionError();
                }
                ...
                if (!undoRedo.getUndoStack().isEmpty()) {
                    ...
                    switch (tabName) {
                        case "Quản lý bộ đề":
                            ...
                            break;
                        case "Đăng ký thi":
                            GiaoVienDangKy gvdk = (GiaoVienDangKy) objectActionUndo.getObjects().get(0);
                            row = new Object[]{
                                gvdk.getMaGV(), gvdk.getMaLop(), gvdk.getMaMH(),
                                gvdk.getTrinhDo(), new SimpleDateFormat("dd/MM/yyyy HH:mm").format(gvdk.getThoiGian()),
                                gvdk.getLan(), gvdk.getSoCauThi(), gvdk.getThoiGian()
                            };
                            break;
                ...
                if (!undoRedo.getRedoStack().isEmpty()) {
                    ...
                    switch (tabName) {
                        case "Quản lý bộ đề":
                            ...
                            break;
                        case "Đăng ký thi":
                            GiaoVienDangKy gvdk = (GiaoVienDangKy) objectActionRedo.getObjects().get(0);
                            row = new Object[]{
                                gvdk.getMaGV(), gvdk.getMaLop(), gvdk.getMaMH(),
                                gvdk.getTrinhDo(), new SimpleDateFormat("dd/MM/yyyy HH:mm").format(gvdk.getThoiGian()),
                                gvdk.getLan(), gvdk.getSoCauThi(), gvdk.getThoiGian()
...
//                Reset form nhập liệu và load lại dữ liệu
                switch (tabName) {
                    case "Quản lý bộ đề":
//                        Load lại dữ liệu
                        ...
                        break;
                    case "Đăng ký thi":
                        GiaoVienDangKyController.renderData(frmMain, _this);
                        ((PnlTeacherRegisterInfo) pnlObjectInfo).reset();
                        break;
                    default:
                        throw new AssertionError();
                }
...
//                Cập nhật thông tin cột dữ liệu
                columns = new String[]{
                    "Giáo viên đăng ký", "Lớp", "Môn học", "Trình độ",
                    "Ngày thi", "Lần thi", "Số câu thi", "Thời gian thi"
                };
                for (String column : columns) {
                    model.addColumn(column);
                }
...
//                Thêm form thao tác nhập liệu tương ứng
                pnlDataEntryFormContainer.add((pnlObjectInfo = new PnlTeacherRegisterInfo()));
        }
    }
        ...
        switch (tabName) {
            ...
            case "Đăng ký thi":
                for (ObjectAction objectAction : objectActions) {
                    ...
                    row.add(((GiaoVienDangKy) objectAction.getObjects().get(0)).getMaGV());
                    row.add(((GiaoVienDangKy) objectAction.getObjects().get(0)).getMaLop());
                    row.add(((GiaoVienDangKy) objectAction.getObjects().get(0)).getMaMH());
                    row.add(((GiaoVienDangKy) objectAction.getObjects().get(0)).getTrinhDo());
                    row.add(simpleDateFormat.format(((GiaoVienDangKy) objectAction.getObjects().get(0)).getNgayThi()));
                    row.add(((GiaoVienDangKy) objectAction.getObjects().get(0)).getLan());
                    row.add(((GiaoVienDangKy) objectAction.getObjects().get(0)).getSoCauThi());
                    row.add(((GiaoVienDangKy) objectAction.getObjects().get(0)).getThoiGian());
                    dataModel.addRow(row.toArray());
                }
                break;
            default:
    ...
    public void fillObjectInfoForm(Object object) throws SQLException {
        switch (tabName) {
            ...
            case "Đăng ký thi": {
                PnlTeacherRegisterInfo pnlTeacherRegisterInfo = (PnlTeacherRegisterInfo) pnlObjectInfo;
                GiaoVienDangKy gvdk = (GiaoVienDangKy) object;
                pnlTeacherRegisterInfo.getTxtTeacherCode().setText(gvdk.getMaGV());
                pnlTeacherRegisterInfo.setSelectedClassroomItem(gvdk.getMaLop());
                pnlTeacherRegisterInfo.setSelectedSubjectItem(gvdk.getMaMH());
                pnlTeacherRegisterInfo.getCboLevel().setSelectedItem(gvdk.getTrinhDo());
                pnlTeacherRegisterInfo.getDateTime().setDateTime(gvdk.getNgayThi());
                pnlTeacherRegisterInfo.getCboTimes().setSelectedItem(String.valueOf(gvdk.getLan()));
                pnlTeacherRegisterInfo.getCboQuestionQuantity().setSelectedItem(String.valueOf(gvdk.getSoCauThi()));
                pnlTeacherRegisterInfo.getCboTime().setSelectedItem(String.valueOf(gvdk.getThoiGian()));
                break;
            }
            default:
                throw new AssertionError();
    ...
    public Object getObjectFromRow(int rowIndex) {
        ...
        switch (tabName) {
            ...
            case "Đăng ký thi": {
                try {
                    object = new GiaoVienDangKy(
                            model.getValueAt(rowIndex, 0).toString(),
                            model.getValueAt(rowIndex, 1).toString(),
                            model.getValueAt(rowIndex, 2).toString(),
                            model.getValueAt(rowIndex, 3).toString(),
                            new SimpleDateFormat("dd/MM/yyyy HH:mm").parse(model.getValueAt(rowIndex, 4).toString()),
                            Short.parseShort(model.getValueAt(rowIndex, 5).toString()),
                            Short.parseShort(model.getValueAt(rowIndex, 6).toString()),
                            Short.parseShort(model.getValueAt(rowIndex, 7).toString())
                    );
                    break;
                } catch (ParseException ex) {
                    MessageBox.showErrorBox(ex.getClass().getPackageName(), ex.getMessage());
                }
            }
            default:
                object = null;
