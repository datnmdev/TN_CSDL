...
import com.ptithcm.tracnghiem.controller.LopController;
import com.ptithcm.tracnghiem.controller.MonHocController;
...
//        Sự kiện với nút quản lý bộ đề
        pnlMenu.getBtnQuestionManage().addMouseListener(new MouseAdapter() {
         ...
         pnlMenu.getBtnExamCalendarRegister().addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                String tabName = pnlMenu.getBtnExamCalendarRegister().getName();
                int tabIndex = tabbedPaneWorkspace.indexOfTab(tabName);
                if (tabIndex != -1) {
                    tabbedPaneWorkspace.setSelectedIndex(tabIndex);
                } else {
                    try {
                        ...
                        tabbedPaneWorkspace.setTitleAt(tabbedPaneWorkspace.getTabCount()-1, tabName);
                        tabbedPaneWorkspace.setSelectedIndex(tabbedPaneWorkspace.getTabCount()-1);
                        GiaoVienDangKyController.renderData(_this, pnlWorkSection);
                    } catch (IOException ex) {
                        MessageBox.showErrorBox(ex.getClass().getName(), ex.getMessage());
                    }
                }
        ...
        pnlMenu.getBtnDoExam().addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                String tabName = pnlMenu.getBtnDoExam().getName();
                int tabIndex = tabbedPaneWorkspace.indexOfTab(tabName);
                if (tabIndex != -1) {
                    tabbedPaneWorkspace.setSelectedIndex(tabIndex);
                } else {
                    PnlExam pnlExam = new PnlExam(_this);
                    tabbedPaneWorkspace.addTab(tabName, pnlExam);
                    tabbedPaneWorkspace.setTitleAt(tabbedPaneWorkspace.getTabCount()-1, tabName);
                    tabbedPaneWorkspace.setSelectedIndex(tabbedPaneWorkspace.getTabCount()-1);
                }
            }
        ...
        pnlMenu.getBtnReviewExamResult().addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                String tabName = pnlMenu.getBtnReviewExamResult().getName();
                int tabIndex = tabbedPaneWorkspace.indexOfTab(tabName);
                if (tabIndex != -1) {
                    tabbedPaneWorkspace.setSelectedIndex(tabIndex);
                } else {
...
//    Getters and setters
    ...
    public FrmMain getThis() {
        return _this;
    }

    public void setThis(FrmMain _this) {
        this._this = _this;
    }

    public PnlKhoaManager getPnlKhoaManager() {
        return pnlKhoaManager;
    }

    public void setPnlKhoaManager(PnlKhoaManager pnlKhoaManager) {
        this.pnlKhoaManager = pnlKhoaManager;
    }
    ...
    public JPanel getPnlMenuContainer() {
        return pnlMenuContainer;
    }

    public void setPnlMenuContainer(JPanel pnlMenuContainer) {
        this.pnlMenuContainer = pnlMenuContainer;
    }

    public JPanel getPnlStatusbarContainer() {
        return pnlStatusbarContainer;
    }

    public void setPnlStatusbarContainer(JPanel pnlStatusbarContainer) {
        this.pnlStatusbarContainer = pnlStatusbarContainer;
    }

    public JPanel getPnlWorkspaceContainer() {
        return pnlWorkspaceContainer;
    }

    public void setPnlWorkspaceContainer(JPanel pnlWorkspaceContainer) {
        this.pnlWorkspaceContainer = pnlWorkspaceContainer;
    }
